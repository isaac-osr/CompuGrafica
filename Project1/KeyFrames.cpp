...
#define MAX_FRAMES 9
int i_max_steps = 190;
int i_curr_steps = 0;//keyframe en el queestamos
...
void saveFrame(void)//guarda el frame, primero el indice, luego su posicion
{
	...
	//prueba
	KeyFrame[FrameIndex].rotHead = head;
...
void interpolation(void)
{
	...
	//prueba
	KeyFrame[playIndex].incrotHead =(KeyFrame[playIndex + 1].rotHead - KeyFrame[playIndex].rotHead) / i_max_steps;
...
int main()
{
	...
	//KeyFrames
	for (int i = 0; i < MAX_FRAMES; i++)//se inicializa
	{
...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if (keys[GLFW_KEY_L])
	{
		if (play == false && (FrameIndex > 1))//pregunta si play está en falso y si tenemos más de un frame
		{
...
void Animation() {
	...
	if (play)
	{
		if (i_curr_steps >= i_max_steps) //end of animation between frames?. frame actual
		{
			playIndex++;
		...
		else
		{
			...
			//prueba
			head += KeyFrame[playIndex].incrotHead;
			i_curr_steps++;
		}
...
Ejercicio 1: a la cabeza aguerar con la tecla 4 y 5 que la cabeza rote y sea aguegada a los keyframes
*/